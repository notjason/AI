...
class ActionLayer(BaseActionLayer):
    ...
    def _inconsistent_effects(self, actionA, actionB):
        ...
        layers.ActionNode
        """
        #Check if Effect in one action neagtes another
    ...
    def _interference(self, actionA, actionB):
        ...
        layers.ActionNode
        """
        for PA in actionA.preconditions:
            for EB in actionB.effects:
                if EB == ~PA:
                    return True
        for PB in actionB.preconditions:
            for EA in actionA.effects:
                if EA == ~PB:
                    return True
        return False
...
class PlanningGraph:
    ...
    def h_levelsum(self):
        ...
        while len(found) < len(self.goal):
            last_literal_layer = self.literal_layers[-1]
            for literal in last_literal_layer:
                if literal in self.goal and literal not in found:
                    found.append(literal)
                    level_sum += len(self.literal_layers) - 1 ## level is zero-indexed
            self._extend()
